...
func (uc *resource) GetAccessibleGroupUUIDs(c *gin.Context, userUUID string) ([]string, error) {
	INFO(Mcode(SRNRSR1), "GetAccessibleGroupUUIDs called")
	...
	if err != nil {
		ERROR(Mcode(SRNRSR2), "Failed to list members")
		return nil, err
	}
	...
	INFO(Mcode(SRNRSR1), "GetAccessibleGroupUUIDs succeeded")
...
func (uc *resource) getMemberForUserAndGroup(c *gin.Context, userUUID, groupUUID string) (*model.Members, error) {
	INFO(Mcode(SRNRSR1), "getMemberForUserAndGroup called")
	filter := repository.MemberQueryFilter{
		UserUUID:  &userUUID,
		GroupUUID: &groupUUID,
		Limit:     1,
	...
	if len(members) == 0 {
		INFO(Mcode(SRNRSR1), "No member found")
		return nil, nil
	}
	INFO(Mcode(SRNRSR1), "getMemberForUserAndGroup succeeded")
...
func (uc *resource) GetResourcesAccessible(c *gin.Context, userUUID string, filter model.ResourceQueryFilter) ([]model.Resources, error) {
	INFO(Mcode(SRNRSR1), "GetResourcesAccessible called")
	...
	if err != nil {
		ERROR(Mcode(SRNRSR2), "Failed to get accessible groups")
		return nil, err
	}
	if len(groups) == 0 {
		INFO(Mcode(SRNRSR1), "No accessible groups found")
		return []model.Resources{}, nil
	}
	filter.GroupUUIDs = groups
	resources, err := uc.resourceRepo.GetResources(c, filter)
	if err != nil {
		ERROR(Mcode(SRNRSR2), "Failed to get resources")
		return nil, err
	}
	INFO(Mcode(SRNRSR1), "GetResourcesAccessible succeeded")
...
func (uc *resource) GetResourceAccessible(c *gin.Context, userUUID string, id string) (*model.Resources, error) {
	INFO(Mcode(SRNRSR1), "GetResourceAccessible called")
	...
	for _, g := range groups {
		if g == res.GroupUUID {
			INFO(Mcode(SRNRSR1), "GetResourceAccessible succeeded")
			return res, nil
		}
	}
...
func (uc *resource) CountResourcesAccessible(c *gin.Context, userUUID string, filter model.ResourceQueryFilter) (int64, error) {
	INFO(Mcode(SRNRSR1), "CountResourcesAccessible called")
	...
	INFO(Mcode(SRNRSR1), "CountResourcesAccessible succeeded")
	return count, nil
}
...
func (uc *resource) CreateResourceWithAccess(c *gin.Context, userUUID string, resource *model.Resources) error {
	INFO(Mcode(SRNRSR1), "CreateResourceWithAccess called")
	if resource.GroupUUID == "" {
		ERROR(Mcode(SRNRSR2), "GroupUUID is required")
		return errors.New("group_uuid required")
	}
	member, err := uc.getMemberForUserAndGroup(c, userUUID, resource.GroupUUID)
	if err != nil {
		ERROR(Mcode(SRNRSR2), "Failed to get member")
		return err
	}
	if member == nil {
		ERROR(Mcode(SRNRSR2), "Access denied - not a member")
		return errors.New("access denied")
	}
	if member.Role != "owner" && member.Role != "editor" {
		ERROR(Mcode(SRNRSR2), "Insufficient role to create resource")
	...
	INFO(Mcode(SRNRSR1), "CreateResourceWithAccess succeeded")
	return nil
}
...
func (uc *resource) UpdateResourceWithAccess(c *gin.Context, userUUID string, resource *model.Resources) error {
	INFO(Mcode(SRNRSR1), "UpdateResourceWithAccess called")
	// Ensure resource exists
	existing, err := uc.resourceRepo.GetResource(c, resource.UUID)
	if err != nil {
		ERROR(Mcode(SRNRSR2), "Failed to get existing resource")
		return err
	}
	...
	if member.Role != "owner" && member.Role != "editor" {
		ERROR(Mcode(SRNRSR2), "Insufficient role to update resource")
	...
	INFO(Mcode(SRNRSR1), "UpdateResourceWithAccess succeeded")
	return nil
}
...
func (uc *resource) DeleteResourceWithAccess(c *gin.Context, userUUID string, id string) error {
	INFO(Mcode(SRNRSR1), "DeleteResourceWithAccess called")
	...
	if member.Role != "owner" && member.Role != "editor" {
		ERROR(Mcode(SRNRSR2), "Insufficient role to delete resource")
	...
	INFO(Mcode(SRNRSR1), "DeleteResourceWithAccess succeeded")
	return nil
}
